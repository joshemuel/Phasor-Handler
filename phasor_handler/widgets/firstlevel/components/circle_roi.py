...
class CircleRoiTool(QObject):
        ...
        if getattr(self, '_show_stim_rois', True):
            try:
                ...
                for stim_roi in list(self._stim_rois or []):
                    try:
                        graphic_type = stim_roi.get('graphic_type', 8)  # Default to rectangle
                        ...
                        if graphic_type == 3:
                            # Polygon/Freehand - draw polygon from points
                            points = stim_roi.get('points')
                            ...
                            for img_x, img_y in points:
                                label_pt = self._image_to_label_coords(img_x, img_y)
                                if label_pt is not None:
                                    label_points.append(label_pt)
                            
                            if len(label_points) >= 3:
                                # Draw polygon
                                from PyQt6.QtCore import QPointF
                                from PyQt6.QtGui import QPolygonF
                                qpoints = [QPointF(x - offset_x, y - offset_y) for x, y in label_points]
                                polygon = QPolygonF(qpoints)
                                painter.drawPolygon(polygon)
                                
                                # Calculate center for label
                                if getattr(self, '_show_labels', True):
                                    cx = sum(p.x() for p in qpoints) / len(qpoints)
                                    cy = sum(p.y() for p in qpoints) / len(qpoints)
                                    self._draw_stim_label(painter, cx, cy, stim_roi, offset_x, offset_y)
                        
                        elif graphic_type == 4:
                            # Ellipse - draw ellipse in bounding box
                            ...
                            if getattr(self, '_show_labels', True):
                                tx = float(px0 + lw / 2.0)
                                ty = float(py0 + lh / 2.0)
                                self._draw_stim_label(painter, tx, ty, stim_roi, offset_x, offset_y)
                            ...
                            # Unknown type - default to ellipse for backward compatibility
                            xyxy = stim_roi.get('xyxy')
                            if xyxy is None:
                                continue
                            lbbox = self._label_bbox_from_image_xyxy(xyxy)
                        ...
                        print(f"DEBUG: Error drawing stim ROI: {e}")
                        import traceback
                        traceback.print_exc()
                        continue
            except Exception:
    ...
    def _image_to_label_coords(self, img_x, img_y):
        """Convert image coordinates to label coordinates.
        ...
        Returns:
            Tuple of (label_x, label_y) or None if conversion fails
        """
        if self._draw_rect is None or self._img_w is None or self._img_h is None:
            return None
        ...
        try:
            ...
            nx = float(img_x) / max(1.0, float(self._img_w))
            ny = float(img_y) / max(1.0, float(self._img_h))
            
            lx = float(self._draw_rect.left() + nx * pw)
            ly = float(self._draw_rect.top() + ny * ph)
            
            return (lx, ly)
        except Exception:
            return None
    
    def _draw_stim_label(self, painter, tx, ty, stim_roi, offset_x=0, offset_y=0):
        """Draw a stimulus ROI label at the given position.
        
        Args:
            painter: QPainter instance
            tx: X coordinate for text center
            ty: Y coordinate for text center
            stim_roi: Dictionary containing ROI data
            offset_x: X offset for scrolling (default 0)
            offset_y: Y offset for scrolling (default 0)
        """
